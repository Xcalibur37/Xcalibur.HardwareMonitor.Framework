...
internal class RyzenSMU
{
    ...
    public float[] GetPmTable()
    {
        ...
        if (table.Length != 0 && table[0] != 0) return table;
        Thread.Sleep(10);
        TransferTableToDram();
    ...
    /// <summary>
    /// Gets the pm table structure.
    /// </summary>
    /// <returns></returns>
    public Dictionary<uint, SmuSensorType> GetPmTableStructure() =>
        !IsPmTableLayoutDefined() ? new Dictionary<uint, SmuSensorType>() : _supportedPmTableVersions[_pmTableVersion];

    /// <summary>
    /// Determines whether [is pm table layout defined].
    /// </summary>
    /// <returns>
    ///   <c>true</c> if [is pm table layout defined]; otherwise, <c>false</c>.
    /// </returns>
    public bool IsPmTableLayoutDefined() => _supportedPmTableVersions.ContainsKey(_pmTableVersion);

    /// <summary>
    /// Transfers the table to dram.
    /// </summary>
    /// <returns></returns>
    ...
    /// <returns></returns>
    private static bool ReadReg(uint address, ref uint data)
    {
        bool read = false;
    ...
    private bool SendCommand(uint msg, ref uint[] args)
    {
        ...
        uint tmp = 0;
        if (!Mutexes.WaitPciBus(5000)) return false;
    ...
    /// <summary>
    /// Setups the size of the pm table addr and.
    /// </summary>
    /// <returns></returns>
    ...
    /// <summary>
    /// Setups the size of the pm table.
    /// </summary>
    private void SetupPmTableSize()
    {
        if (!GetPmTableVersion(ref _pmTableVersion)) return;
